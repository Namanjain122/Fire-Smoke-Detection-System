...
os.system("title Static_Fire_Detection: 5005")
# Initialize Flask
app = Flask(__name__)
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        if ext == "zip":
            unique_name = get_unique_name("zip")
            ...
            for extracted_file in Path(UPLOAD_FOLDER).rglob("*"):
                if extracted_file.is_file() and allowed_file(extracted_file.name):
                    ...
                    if sub_ext in ["png", "jpg", "jpeg"]:
                        model(str(extracted_file), save=True, project=RESULTS_FOLDER,
                              name=unique_name, exist_ok=False)

                    # Video inside ZIP
                    elif sub_ext in ["mp4", "avi", "mov", "mkv"]:
                        results = model(str(extracted_file), save=True, project=RESULTS_FOLDER,
                                        name=unique_name, exist_ok=False)
                        save_dir = Path(results[0].save_dir)
                        processed_files = list(save_dir.glob("*.avi")) + list(save_dir.glob("*.mp4")) \
                                          + list(save_dir.glob("*.mov")) + list(save_dir.glob("*.mkv"))
                        if processed_files:
        ...
        # 🖼 Handle Image Upload
            ...
            unique_name = get_unique_name("image")
            results = model(file_path, save=True, project=RESULTS_FOLDER,
                            name=unique_name, exist_ok=False)
            save_dir = Path(results[0].save_dir)
        ...
        # 🎥 Handle Video Upload
            ...
            unique_name = get_unique_name("video")
            results = model(file_path, save=True, project=RESULTS_FOLDER,
                            name=unique_name, exist_ok=False)
            save_dir = Path(results[0].save_dir)
            processed_files = list(save_dir.glob("*.avi")) + list(save_dir.glob("*.mp4")) \
